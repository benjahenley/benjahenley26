...
export function ExpandableCardDemo({ locale }: Props) {
  ...
  return (
    <>
      ...
      <ul className="max-w-2xl mx-auto w-full gap-4">
        {SpotifyCards(locale).map((card, index) => (
          <div
            key={`card-${card.title}`}
            ...
            <button className="px-4 py-2 text-sm rounded-full font-bold bg-gray-300 hover:bg-violet-500 dark:hover:bg-emerald-500 hover:text-white text-gray-800 mt-4 md:mt-0">
              {card.ctaText}
            </button>
          </div>
        ))}
