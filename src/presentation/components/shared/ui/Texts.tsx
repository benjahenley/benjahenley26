...
function SectionTitle({ children, className = "", style }: Props) {
  return (
    <h3
      style={style}
      className={`${className} text-2xl md:text-4xl font-bold text-gray-800 dark:text-gray-100`}>
      {children}
    </h3>
  );
}
...
function TextBase({ children, className = "" }: Props) {
  return (
    <p
      className={`${className} text-base lg:text-md  font-normal text-gray-600 dark:text-gray-400 `}>
      {children}
    </p>
  );
}
